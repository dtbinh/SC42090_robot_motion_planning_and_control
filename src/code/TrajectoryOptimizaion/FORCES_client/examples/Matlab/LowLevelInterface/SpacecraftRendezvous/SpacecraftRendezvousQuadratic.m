...
% 
%  min   sum_{i=1}^{N-1} ||Q*(xi-xtarget)||_2 + ||R*(ui-utarget||_2
% xi,ui
%       s.t. x1 = x
...
for i = 1:N
        ...
        stages(i).dims.p = 3;       % number of polytopic constraints
            
        % cost
        stages(i).cost.H = blkdiag(R,Q);
        stages(i).cost.f = [-R*us; -Q*xs];
        ...
        % polytopic bounds
        stages(i).ineq.p.A  = [ zeros(3,nu), Hx ]; 
        stages(i).ineq.p.b  = hx; 
...
for k = 1:kmax
    ...
    if mod(k-1,Ts_MPC) == 0 % only run the controller every Ts_MPC seconds
        ...
        problem = setfield(problem,'D_1', [B, -eye(nx)]);            
        for i = 2:N
            [A, nucopy] = YA_A_matrix(cfg_MPC, nucopy, Ts_MPC*Ts_SIM);
            B = A*[zeros(3);eye(3)];
            problem = setfield(problem,['D_',num2str(i)], [B, -eye(nx)]); 
            problem = setfield(problem,['C_',num2str(i-1)], [zeros(nx,nu), A]); 
        end   
