...
stages.codeoptions['server'] = 'https://forces.embotech.com'
...
plt.step(list(range(0, kmax)), X[0, 0:kmax], where='post', hold=True)
plt.step(list(range(0, kmax)), X[1, 0:kmax], where='post', hold=True)
plt.title('states')
plt.xlim(0, kmax)
...
plt.step(list(range(0, kmax)), U[0, 0:kmax], where='post', hold=True)
plt.title('input')
plt.xlim(0, kmax)
